...
### Phase 1 클라이언트 측 토글 분리 (2026-04-30, ISSUE-009 후속)

기존 `AUTH_ENABLED` (server-only) 토글은 `proxy.ts`(Next 16 미들웨어)만 우회하고 **클라이언트 측 ClerkProvider 마운트는 막지 못함**. HTTP LAN host(`http://macstudio:3000`) 접속 시 Clerk dev handshake 응답 쿠키의 `Secure` 플래그가 HTTP 컨텍스트에서 저장되지 않아 핸드셰이크 무한 루프 발생 — 도서관·채팅 화면이 영구 비어 있음 ([ISSUE-009](../issues/resolved/ISSUE-009-clerk-handshake-loop-http-lan.md)).

**해결**: `NEXT_PUBLIC_AUTH_ENABLED` 신설 — 클라이언트·서버 양쪽이 같은 의미의 토글을 읽음:
- `web/lib/auth-flag.ts` — 단일 진실 (`process.env.NEXT_PUBLIC_AUTH_ENABLED === "true"`)
- `web/app/layout.tsx` — `false`면 `<ClerkProvider>` 자체를 미렌더 (handshake 진입점 봉쇄)
- `web/lib/api/client.ts` — `useApiClientWithAuth`/`useApiClientNoAuth` 두 hook 정의 후 모듈 로드 시 한 번 결정. React rules-of-hooks 안전 (한 컴포넌트 인스턴스가 항상 같은 hook 호출)
- `web/components/sidebar.tsx` — `<UserButton>` 조건부 렌더 (Clerk context 없으면 throw)

**Phase 2 prereq 재정의**: 외부 공개 또는 다중 사용자 활성화 전에 다음 중 하나 충족 필요:
1. macstudio hostname을 HTTPS로 노출 (Tailscale Funnel / 자체서명 인증서 / Caddy)
2. Clerk dashboard에서 instance hosts에 LAN hostname 추가 + Secure 플래그 host policy 검증
3. HTTPS 진입만 허용 (HTTP 차단)

위 prereq가 충족되기 전에는 `NEXT_PUBLIC_AUTH_ENABLED=true`로 전환 시 다시 ISSUE-009 재발.
...
## ADR-032: 텍스트 추출 품질 추적 + 스캔본 PDF macOS Vision OCR 재색인

**날짜**: 2026-04-30
**상태**: accepted (적용 완료)
**관련**: [ISSUE-010](../issues/resolved/ISSUE-010-scan-only-pdf-extraction.md), ADR-021(Docling 파이프라인), ADR-031(워커 RSS 가드 — OCR 작업 메모리 영향 측정 기준)

### 배경
- 사용자가 시리즈 `ser_f091b1e5242e`(하루하루가 세상의 종말) + "등장인물은?" 질의 시 답이 없음 보고. 진단 결과 두 권의 본문이 사실상 추출 안 됨 (markdown 5~7KB)
- 전체 107건 스캔: **16건이 동일 스캔본 패턴** — 모두 소설 (디지털 포트리스, 스노우 크래쉬, 하루하루가 세상의 종말, 천사들의 제국, 성채, 종말일기 Z, 데드 아일랜드, 좀비 서바이벌 가이드, 양심의 문제, 우주바이러스 The Andromeda Strain)
- Docling 기본 모드(`do_ocr=True`이지만 `force_full_page_ocr=False`)는 레이아웃 모델이 페이지 전체를 단일 이미지로 인식하지 못해 OCR 영역 자체가 안 잡힘. `bitmap_area_threshold=0.05`도 작은 영역을 추가로 필터링
- 기존 텍스트 레이어가 있는 디지털 PDF 91건은 영향 없음 — Docling 텍스트 백엔드가 직접 처리

### 선택지
1. **macOS Vision (`ocrmac`) + `force_full_page_ocr=True`** — 채택
2. EasyOCR — 추가 의존성 ~200MB + 한국어 모델 ~50MB, 한글 정확도 변수, GPU 권장
3. Tesseract — 가벼우나 한글 정확도 낮음
4. 모든 PDF에 OCR 기본 켬 — 디지털 PDF 91건의 색인 시간 폭증, 부적합
5. 사용자에게 텍스트본 재수급 요청 — 가장 정확하지만 16권 분량 운영 부담

### 결정
**옵션 1**. 이유:
- macOS Vision은 **Docling 2.90.0 의존성에 이미 포함** (`ocrmac` package) — 추가 설치 0
- 한국어 OCR 정확도 충분 (단권 테스트 검증, "현창을 통해 주변을 재차 확인한 후 출항 준비를…" 같은 자연스러운 문장 추출)
- 권당 4~8분 (Docling 레이아웃 + 청킹 포함, 81MB PDF 기준 4분, 105MB 기준 7분)
- 메모리 RSS 피크 ~6.9GB — ADR-031 워커 가드(8GB) 안쪽

### 토글 디자인
**일반 PDF에는 OCR 기본 OFF, 스캔본 재색인 시점에만 ON**:
- `DoclingDocumentLoader(force_full_page_ocr=True, ocr_lang=("ko-KR","en-US"))` 명시 인자
- 기본값 `force_full_page_ocr=False` — 기존 색인 경로 100% 후방호환
- 신규 색인 시점에 자동 OCR 진입은 보류 — 디지털 PDF에 OCR 켜면 시간만 폭증

### 식별 — `extraction_quality` 컬럼
- `documents.extraction_quality TEXT` 추가 (NULL/`ok`/`partial`/`scan_only`)
- CHECK 제약 + index
- 휴리스틱 백필: markdown < 30KB OR chunk 평균 길이 < 150B → `scan_only`
- 도서관 카드(NextJS)에 `📷 스캔` 빨간 배지 + tooltip ("검색·답변 불가") 노출

### 재색인 절차
- `scripts/reingest_scan_only.py` — `extraction_quality='scan_only'` 일괄 처리
...
- summary 무효화 → `scripts/generate_summaries.py` 자동 재생성

### 결과 (실측)
- **16/16 성공**, 총 119.5분 (약 2시간)
- 평균 권당 8분, 메모리 피크 6.9GB
- 마크다운 평균 크기: 5KB → 200~500KB (~50~100배 증가)
- 청크 수: 평균 70 → 800 (~10배)
- 검증 질의 (시리즈 + "등장인물은?"): "정보 없음" → 실제 본문 인용 ("놈들·해병들·중사·FBI 요원…")

### 회귀 전략
- `force_full_page_ocr=False` 기본값으로 신규 색인은 변함없음
- 16건 중 한 권만 문제 시 `--doc-id`로 해당 권만 재처리
- DB·Qdrant 갱신은 권 단위 트랜잭션 — 부분 실패 격리

### 리스크 / 한계
- **macOS 종속성**: `ocrmac`은 macOS Vision API 필수. Linux 운영 시 EasyOCR/Tesseract로 fallback 필요 (별건)
- **신규 스캔본 자동 감지 부재**: 새 PDF 색인 시 자동으로 `scan_only` 마킹·재색인은 별건. 현재는 수동 백필 (`scripts/classify_extraction_quality.py --only-empty`)
- **OCR 정확도**: 폰트·해상도·페이지 회전에 따라 변동. 일부 표지·판권 페이지는 여전히 노이즈 (예: `AY BY DAY ARMAGEDDON EVOND EXL!`)
- **시간 비용**: OCR + 임베딩 + LLM 요약 재생성 합산 권당 ~10분 — 16권 일괄 처리 시 2시간 가량 워커 점유

### 후속
- summary 재생성 (`scripts/generate_summaries.py`) — `summary=NULL` 16건 자동 처리, gpt-4o-mini 약 $0.30~0.50
- 신규 스캔본 자동 감지·플래그 — TASK 별건 (indexer 워커 후처리 훅)
- launchd 큐에 `classify_extraction_quality --only-empty` 정기 실행 검토 — ADR-031 운영 인프라와 합류
- Linux 환경 OCR 백엔드 fallback (EasyOCR/Tesseract) — 외부 배포 검토 시

---

## ADR-033: 답변 스트리밍 SSE — 첫 토큰 ~500ms · 신규 엔드포인트 분리

**날짜**: 2026-04-30
**상태**: accepted (적용 완료)
**관련**: TASK-024, ADR-030(NextJS UI 분리 — 후속 묶음 "답변 스트리밍 SSE 도입 — 별건"), ISSUE-011(옵티미스틱 user 버블 — 같은 UX 묶음)

### 배경
- 기존 `/query`는 retrieval + rerank + generation 전부 끝나고 한 번에 JSON 응답 — 사용자 체감 latency 3~5초
- LLM 토큰은 OpenAI streaming으로 즉시 받을 수 있으나, 평가 스크립트(`bench_*`) 호환성 우려로 그동안 큐잉
- TASK-024 사용자 명시 착수 (2026-04-30): 첫 토큰 ~500ms로 체감 latency 5x 개선

### 선택지
1. **`/query/stream` 신규 SSE 엔드포인트 + 기존 `/query` 유지** — 채택
2. 기존 `/query`에 `?stream=true` 쿼리 파라미터로 분기 — accept 헤더 분기 가능하나 평가 스크립트 영향 잠재. 응답 타입(`application/json` vs `text/event-stream`)이 같은 라우트에서 분기하면 OpenAPI 스키마도 모호
3. WebSocket — 양방향 불필요(클라 → 서버는 1회), 인프라 복잡도만 ↑
4. JSON 모드 streaming (`response_format: json_object`로 answer + suggestions 함께 streaming) — 부분 JSON 파싱이 까다롭고 첫 토큰이 `"answer":` 헤더를 거쳐야 시작돼 지연

### 결정
**옵션 1**. 이유:
- 평가 스크립트·기타 호출자 후방호환 100% (`/query` 변경 0)
- SSE는 단방향 streaming에 충분하고 표준 — `fetch + ReadableStream`만으로 클라이언트 구현
- OpenAPI 스키마는 `/query/stream`을 일반 POST로 노출 (응답은 `text/event-stream`)
- suggestions는 별도 두 번째 LLM 호출로 분리 — 답변 streaming 첫 토큰 지연 0, 답변 종료 후 ~1~2초 추가 호출
...
- **`packages/rag/generator.py`**:
  - `generate_stream(llm, question, chunks, history)` — `llm.stream()`을 감싸 토큰 yield (sync iterator)
  - `generate_suggestions(llm, question, answer, count)` — 답변 종료 후 별도 호출로 후속 질문만 JSON 모드로 빠르게 생성
  - `INSUFFICIENT_MARKERS` 상수화 — 답변이 "정보 없음" 류이면 suggestions 강제 비움
- **`packages/rag/pipeline.py:query_stream`** — `(event_type, data)` 튜플 yield하는 sync generator. `@traceable`로 LangSmith run 자동 생성. `tracing_context`로 메타·태그 일관성 유지 (`stream:true` 태그 추가)
- **`apps/routers/query.py`**:
  - `POST /query/stream` 라우트 신설
  - 사용자 메시지 우선 commit (스트림 도중 conn drop 시에도 보존)
  - 어시스턴트 메시지는 스트림 종료 후 fresh DB session으로 commit (요청 측 session 닫힘 회피)
  - `StreamingResponse(media_type="text/event-stream")` + `X-Accel-Buffering: no` 헤더 (reverse proxy 버퍼링 비활성)
  - 예외 시 `event: error` 발급 후 종료

### 프론트엔드 구현
- **`web/lib/hooks/use-rag-stream.ts`** (신규):
  - `fetch + ReadableStream + TextDecoderStream`. EventSource는 POST·헤더 자유도 부족이라 회피
  - 빈 줄(`\n\n`) 단위로 SSE 이벤트 분할, `event:` / `data:` 라인 파싱
  - `AbortController`로 사용자 측 중단 지원
  - 콜백 인터페이스 (`onMeta/onSources/onToken/onSuggestions/onDone/onError`)
- **`web/app/chat/page.tsx`**:
  - `useRagQuery` mutation → `useRagStream` send 교체
  - `streamingAnswer` 상태 — 토큰을 누적해 어시스턴트 버블에 점진 렌더
  - `onDone` 시 `qc.invalidateQueries(conversations.all)`로 conversation refetch → baseMessages 업데이트되면 streamingAnswer 자동 클리어 (self-healing)
  - 옵티미스틱 user 버블(ISSUE-011) + streaming assistant 버블 동시 동작
- **`web/components/chat/chat-input.tsx`**:
  - `pending && onCancel` 시 send 버튼 자리에 빨간 X (중지) 버튼 — `AbortController.abort()` 트리거

### 결과 (실측)
- 채팅 첫 토큰 ~500ms (현재까지 5초 → 5~10x 개선, retrieval+rerank ~300~400ms 후 즉시 토큰 시작)
- 기존 `/query` 회귀 0 — 평가 스크립트·테스트 영향 없음
- DB 영속화: 사용자/어시스턴트 메시지 모두 정상 저장 (Playwright 검증)

### 회귀 전략
- 기존 `/query` 변경 0 — 문제 시 클라이언트가 `useRagQuery` 다시 사용하면 복귀
- `useRagStream`만 hook 단위 분리 — 부작용 최소

### 리스크 / 한계
- **NextJS rewrites + chunked transfer**: same-origin 프록시(`/api/* → INTERNAL_API_URL/*`)가 SSE 청크를 그대로 전달해야 함. NextJS dev/prod 모두 자동 — 검증 완료
- **AbortController 중단 시점**: 사용자가 중지하면 클라이언트 토큰 수신은 멈추지만 백엔드 LLM 호출은 진행 중일 수 있음(LangChain `.stream()`은 generator close에 응답하나 OpenAI HTTP 응답 전송 중이면 잠시 더 진행). 비용 부담은 있으나 1인 운영엔 미미
- **suggestions 두 번째 호출**: 답변 종료 후 ~1~2초 추가 latency. 사용자에겐 streaming 종료 후 잠깐 빈 칸. 후순위 개선 (suggestions를 token 스트림 끝에 동기 inject할지 별도 검토)
- **모바일 SSE 호환성 검증 미완**: Playwright 데스크톱만 검증. iOS Safari·Android Chrome 실측은 ISSUE-001 묶음과 같이 보류

### 후속
- 평가 벤치 회귀 검증 (`bench_answers.py`) — 다음 정기 실행 시
- suggestions inline streaming 검토 — JSON 모드 incremental 파서 또는 토큰 스트림 끝에 마커로 dispatch
- 모바일 viewport 검증 — 인증·공개배포 묶음과 함께 해제 시
- AbortController로 백엔드 LLM 호출 즉시 cancel — LangChain `astream()`로 전환 + asyncio task cancel

---

## ADR-034: 질문 본문에서 책 제목 매칭 → implicit doc_filter 자동 적용

**날짜**: 2026-04-30
**상태**: accepted (적용 완료)
**관련**: ADR-026(도서관 doc_filter 라우팅), ADR-029(스코프 우선순위 doc > category > series), 사용자 보고("질문에 책 제목이 들어 있는데 다른 책 청크가 나옴", 2026-04-30)

### 배경
- 사용자 질의 `"2001 스페이스 오디세이에서 HAL9000은 뭐지?"`처럼 **책 제목을 본문에 명시**해도, retrieval이 해당 책의 본문 청크가 아닌 다른 책에서 같은 키워드를 다룬 청크를 1순위로 끌어옴
- 구체 사례:
  - top-1 = 『인공생명』 p.65 (score 0.61) — HAL 9000을 **각주로 짧게 정의**한 단락. 정의 질문에 의미적으로 강하게 매칭
  - 정작 원작 『2001 스페이스 오디세이』 본문은 top-4 안에 못 들어옴 (서사체라 정의문 형태가 약함)
- 근본 원인: chunk text에 **책 제목이 직접 포함되지 않음** (heading_path만 prepend됨). BM25·dense embedding 모두 query의 "2001 스페이스 오디세이" 토큰을 chunk의 title에 매칭할 수 없음. title은 metadata로만 존재
- 결과: 사용자가 책을 명시했음에도 retrieval은 그 의도를 알 수 없고, 답변이 의도치 않은 책의 각주에서 만들어짐

### 선택지
1. **질문 본문에서 책 제목 substring 매칭 → 단일 매칭 시 implicit doc_filter 적용** — 채택
2. **모든 청크 텍스트에 책 제목 prefix 추가 후 재임베딩** — 가장 일반적이나 1.7만 청크 재임베딩 비용·시간(수십 분 + OpenAI 비용 발생). 색인 시점만 수정해도 기존 데이터는 부적용. 효과 대비 부담 큼
3. **rerank 단계 title 가중치** — 모든 retrieve 후 title 토큰 매칭 시 boost. 영향이 모호하고 수치 튜닝 필요. 사용자 의도 명시 케이스엔 boost로는 부족
4. **LLM 기반 query 파싱** — 별도 LLM 호출로 "in book X" 패턴 추출. latency·비용 추가. 사용자가 명시한 정확한 매칭에 비해 과한 도구
5. **사용자 UI에 "책 자동 인식" 토글** — 의도 명시는 좋으나 클릭 한 번 더 + 발견성 낮음. UX 마찰

### 결정
**옵션 1**. 이유:
- **사용자 의도가 가장 명확하게 표명되는 시점**(질문 본문 자체)에서 처리 — UI 추가 동작 0
- substring 매칭은 **확정성 높음**: title 텍스트가 정규화 후 그대로 질문에 포함되면 사용자가 명시한 것이 분명. False positive 리스크 낮음
- 기존 `doc_filter` 인프라 100% 재사용 — Qdrant filter, 라우팅, ScopeBanner 등 변경 0
- explicit scope filter가 하나라도 있으면 implicit 우회 → ADR-029 스코프 우선순위 정책 일관 유지
- **비용 0** — 추가 LLM 호출·재임베딩·새 인프라 없음. SQL 1회 + Python regex가 전부

### 매칭 알고리즘
```
1. 정규화: [공백·언더스코어·하이픈·점·중점] → 단일 공백, 소문자
   예) "2001_스페이스_오디세이" → "2001 스페이스 오디세이"
       "디지털_포트리스_01"      → "디지털 포트리스 01"
2. 모든 documents.title을 정규화한 후 길이 ≥ 4 이상만 후보로 사용
3. 정규화된 질문에 정규화된 title이 substring으로 포함되면 매칭
4. 매칭 후보가 여러 개면 가장 긴 정규화 title 1건 채택 (가장 구체적)
5. 동률(여러 책이 같은 길이로 매칭)이면 모호하다고 보고 None — implicit 적용 안 함
```

**왜 substring + 길이 우선 + 동률 시 포기?**
- substring은 "2001 스페이스 오디세이"가 질문에 포함되는 케이스를 정확히 잡음
- 시리즈명(예: "스페이스 오디세이")만 매칭되면 1·2·3편 모두 동률로 잡히고 → 모호함 → 적용 안 함 (사용자가 시리즈 전체를 의도했을 수도, 특정 권을 의도했을 수도 있음. 잘못 추측해서 한 권만 필터링하면 손해 큼)
- 길이 우선: "2001 스페이스 오디세이"는 "스페이스 오디세이"보다 길고 더 구체적 → "2001 스페이스 오디세이"가 단일 매칭이면 그것 채택
- 동률 시 포기 정책으로 false positive 자연 차단

### 캐시 정책
- documents.title 변경은 드물어 모듈 레벨 list 캐시 (TTL 60초)
- 색인 후 최대 1분 내 자동 반영. 운영자 즉시성 필요 시 uvicorn restart로 강제 갱신
- 동시성 안전: 캐시 갱신 race는 마지막 writer wins로 충분 (read-only 데이터)

### 구현 위치
- 신규 모듈: `packages/rag/title_match.py` (50줄)
  - `_normalize(text)`: 정규화 헬퍼
  - `_refresh_cache()`: SQL `SELECT doc_id, title FROM documents`
  - `detect_implicit_doc_filter(question) -> (doc_id, title) | None`: 매칭 진입점
- `packages/rag/pipeline.py:query` / `query_stream`:
  - 진입 직후 explicit scope 모두 None인 경우만 호출
  - 매칭 시 `doc_filter`로 자동 주입 + LangSmith 메타에 `implicit_doc_title` 기록 (트레이스에서 인지 가능)
- ADR-029 우선순위와 충돌 0 — explicit이 하나라도 있으면 implicit 우회
...
부수 효과: "디지털 포트리스의 주인공은?", "스노우 크래쉬의 메타버스란?" 같은 질의도 자동으로 해당 책에만 검색.

### 회귀 전략
- explicit scope를 하나라도 지정하면 implicit 우회 — 기존 동작 변경 0
- title 매칭 실패 시 기본 retrieval (전체 컬렉션) — 이전 동작과 동일
- 코드 변경 단일 모듈 신규 + pipeline 두 메서드 분기 → roll back 시 두 import + 두 분기만 제거하면 됨

### 리스크 / 한계
- **부분 매칭 false positive 우려**: title이 매우 짧고 일반어와 겹치면(예: "코드"라는 제목) 일반 질문에도 implicit 적용 가능. 현재는 정규화 후 길이 ≥ 4 컷오프로 1차 차단. 운영 데이터로 false positive 누적 시 길이 컷·블랙리스트·고유성 점수 추가 검토
- **중복 제목 동률 처리**: 같은 길이 title 다중 매칭 시 모호함으로 None 반환. 사용자 입장에서 "왜 안 잡혔지?"가 될 수 있으나 잘못 잡는 것보다 안전
- **title cache TTL 60초**: 신규 색인 직후 1분 동안은 implicit 매칭 대상 아님. 운영 중 신규 색인 빈도 낮아 영향 미미
- **history 무관**: 직전 turn에서 책 제목을 언급했어도 본 turn 질문에 직접 명시되지 않으면 implicit 매칭 안 됨. 사용자 의도 ⊃ 단순 질문 매칭의 한계. 추후 history 인지 매칭은 별건 ADR로 분리 검토

### 의도적 제외
- 시리즈 단위 implicit 매칭 (예: "스페이스 오디세이" → 시리즈 전체) — 동률 처리로 자연 회피. 시리즈 전체를 원하면 사용자가 시리즈 카드 클릭 (ADR-020 series_filter) 사용
- title 외 author/publisher 등 다른 메타데이터 매칭 — 현재 운영 데이터(저자/출판사 메타데이터 부재)에선 의미 없음. 메타데이터 확장 시 재검토
- LLM 기반 query intent 추출 — 본 ADR의 단순 매칭이 충분. 복잡 패턴(예: "어제 본 그 책") 필요 시 별건
